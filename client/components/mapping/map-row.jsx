...
import setMobileMapRowPosition from '../../../lib/client/actions/set-mobile-map-row-position.js';
  ...
  return (
    <tr id={location._id} onClick={() => {return dispatch(setMapTableRowClick({item: item, coord: location.location}))}} onMouseOver={() => {return dispatch(setMapTableHover(item))}} onMouseOut={() => {return dispatch(setMapTableHover(-1))}}>
      ...
      <td>
        ...
        <Waypoint scrollableAncestor={document.getElementById('map-table')} key={item} onEnter={_onWaypointEnter} onLeave={_onWaypointLeave} onPositionChange={_onWaypointPositionChange} fireOnRapidScroll={true} topOffset='40px'/>
      </td>
    </tr>
  );
};
